...
describe('Part A: Records', () => {
  it('declares a record schema and instantiates with named arguments', () => {
    ...
    if (p.type === 'record') {
      ...
      expect(p.fields.mass || p.fields[':mass']).toEqual({ type: 'rational', n: 2n, d: 1n });
      expect(p.fields.position || p.fields[':position']).toEqual({
        type: 'tuple',
        elements: [
  ...
  it('errors on invalid field access naming the record type and available fields', () => {
    ...
    expect(() => evaluate('p.:charge', env)).toThrowError(/Field '(:?charge)' does not exist on record 'Particle'/);
  });
  ...
  it('updates records immutably using with { ... } returning a new record', () => {
    ...
    if (p2.type === 'record') {
      const vel = p2.fields.velocity || p2.fields[':velocity'];
      expect((vel as any).elements[0]).toEqual({ type: 'rational', n: 2n, d: 1n });
    }
  ...
  it('Gate A: a record with a Vector field kind-checks', () => {
    ...
    if (recordKind.name === 'Record') {
      ...
      const massField = recordKind.fields.mass || recordKind.fields[':mass'];
      const posField = recordKind.fields.position || recordKind.fields[':position'];
      const velField = recordKind.fields.velocity || recordKind.fields[':velocity'];
      expect(massField).toEqual({ name: 'Scalar', subtype: 'natural' });
      expect(posField).toEqual({ name: 'Vector', dimension: 3, baseField: 'R' });
      expect(velField).toEqual({ name: 'Vector', dimension: 3, baseField: 'R' });
    }
    ...
    if (kindOfP.type === 'kind') {
      ...
      expect(formatKind(kindOfP.kind)).toContain('Vector(dim=3, field=R)');
    }
