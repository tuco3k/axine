...
describe('Part B: Derivations as First-Class Values & Self-Verification', () => {
  ...
  it('simplify(3x + 2x - 4, in: x) simplifies polynomials', () => {
    const env = createInitialEnvironment();
    ...
    // 5x - 4
    if (res.type === 'string') {
      expect(res.value.replace(/\s+/g, '')).toBe('5*x-4');
    }
  });

  it('simplify((x^2 - 1)/(x - 1), in: x) simplifies with domain condition x ≠ 1', () => {
    const env = createInitialEnvironment();
    ...
    expect(res).toBeDefined();
    if (res.type === 'string') {
      expect(res.value).toContain('x + 1');
      expect(res.value).toContain('x ≠ 1');
    }
  });
