...
describe('Part G: Three Domain Libraries and Worked Problems (Gate G)', () => {
  it('loads and runs physics.ax with physics_problem.ax', () => {
    const filePath = path.resolve(process.cwd(), 'documents', 'physics_problem.ax');
  ...
  it('loads and runs statistics.ax with statistics_problem.ax', () => {
    const filePath = path.resolve(process.cwd(), 'documents', 'statistics_problem.ax');
    const source = fs.readFileSync(filePath, 'utf-8');
  ...
  it('loads and runs linear.ax with linear_problem.ax', () => {
    const filePath = path.resolve(process.cwd(), 'documents', 'linear_problem.ax');
    const source = fs.readFileSync(filePath, 'utf-8');
